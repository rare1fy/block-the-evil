...
public enum ItemCornerEnum
    ...
    public class ItemPool
    {
        ...
        //====================================
        Action<GameObject, LoopListView2> mItemClickFun;
        bool mIsGrid = false;
        private LoopListView2 mList;
        //====================================

        public ItemPool(Action<GameObject, LoopListView2> itemClickFun, bool isGrid, LoopListView2 list)
        {
            mItemClickFun = itemClickFun;
            mIsGrid = isGrid;
            mList = list;
        }
    ...
    public LoopListViewItem2 CreateItem()
    {
        ...
        if (tViewItem == null)
        {
            tViewItem = go.AddComponent<LoopListViewItem2>();
        }
        tViewItem.ItemPrefabName = mPrefabObj.name;
        ...
        if (mItemClickFun != null)
        {
            ...
            {
                // 父物体的点击禁用
                var parentBtn = go.GetComponent<Button>();
                if (!ReferenceEquals(parentBtn, null))
                {
                    UnityEngine.Object.Destroy(parentBtn);
                }
                var ub = go.GetComponent<UIBase>();
                if (ReferenceEquals(ub, null))
                {
                    SDDebug.LogError("called CreateItem but get UIBase is null from newItem!");
                    return null;
                }
                foreach (var target in ub.TargetContainer)
                {
                    var nodeGo = target.goNode;
                    var itemBtn = nodeGo.GetComponent<Button>();
                    if (ReferenceEquals(itemBtn, null))
                    {
                        itemBtn = nodeGo.AddComponent<Button>();
                    ...
                    if (!ReferenceEquals(itemBtn, null))
                    {
                        ...
                        {
                            mItemClickFun(nodeGo, mList);
                        });
                    }
                    var item = nodeGo.GetComponent<LoopListViewItem2>();
                    if (ReferenceEquals(item, null))
                    {
                        item = nodeGo.AddComponent<LoopListViewItem2>();
                        item.ParentListView = mList;
                    }
                }
            }
            else
            {
                var itemBtn = go.GetComponent<Button>();

                if (ReferenceEquals(itemBtn, null))
                {
                    itemBtn = go.AddComponent<Button>();
                    itemBtn.transition = Selectable.Transition.None;
                }
                if (!ReferenceEquals(itemBtn, null))
                {
        ...
        void RecycleItemReal(LoopListViewItem2 item)
        {
            ...
            if (null != item.RecycleFunc)
            {
                item.RecycleFunc();
                item.RecycleFunc = null;
            }
        }
...
public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    void Start()
    {
        ...
        if (mScrollRect)
        {
            var lg = mScrollRect.content.GetComponent<LayoutGroup>();
            if (lg)
            {
                lg.enabled = false;
            }
    ...
    public Action<GameObject, LoopListView2> itemClickFun; // item点击事件
    /// <summary>
    /// 列表填充事件
    /// </summary>
    /// <param name="index">填充go的数据的index</param>
    /// <param name="go">填充的物体对象</param>
    /// <param name="groupIndex">多个item组成的一个item组在列表的index</param>
    public delegate void ItemRendener(int index, GameObject go, int groupIndex);
    public ItemRendener itemRendener; // 填充item事件
    public Action<float, float, float> scrollRectChgValue; // 位置改变回调，可用来做OnScroll

    private const float scrollRectMaxVec = 2000f;

    [SerializeField, Header("item和list首间距")]
    private float mListStartOffset = 0;

    [SerializeField, Header("item和list尾间距")]
    private float mListEndOffset = 0;
    /*SetListItemCount --> FillSuperList*/

    [SerializeField, Header("是否是网格形式")]
    private bool mIsGrid = false;

    [SerializeField, Header("是否自动适配个数")]
    private bool _isVariable = false;

    [SerializeField, Header("itemGroup中item的间距")]
    private float mItemGroupSpacing = 0;

    [SerializeField, Header("列表单个cell的item个数，自动适配每组个数的需要自己计算")]
    public int mItemCountPerCell = 1;
    /// <summary>
    /// 列表渲染数据总数
    /// </summary>
    private int numDatas = 0;

    private int LogicNumItems(int numItems)
    {
        int _numItems = numItems;

        if (mIsGrid)
        {
            _numItems = numItems / mItemCountPerCell;
            if (numItems % mItemCountPerCell > 0)
            {
                _numItems++;
            }
        }
    ...
    /// <summary>
    /// 因为grid的关系 这里要根据data的index重新计算item的index
    /// </summary>
    /// <param name="dataIndex">渲染数据的index 非list item index</param>
    /// <returns></returns>
    private int LogicMovePanelItemIndex(int dataIndex)
    {
        int index = dataIndex;
        ...
        {
            dataIndex++;
            index = dataIndex / mItemCountPerCell;
            if (dataIndex % mItemCountPerCell > 0)
            {
                index++;
            }
            if (index > 0)
    ...
    {
        SetSnapTargetItemIndex(CurSnapNearestItemIndex - 1);
    }
    ...
    public float ScrollValue()
    {
        ...
        if (mIsVertList)
        {
            if (mScrollRect.verticalScrollbar)
            {
                value = mScrollRect.verticalScrollbar.value;
            }
            else
            {
                SDDebug.LogError("列表调用滑动进度出错 进度条为空...");
            }
        }
        else
        {
            if (mScrollRect.horizontalScrollbar)
            {
                value = mScrollRect.horizontalScrollbar.value;
            }
            else
            {
                SDDebug.LogError("列表调用滑动进度出错 进度条为空...");
    ...
    /// item对象池字典
    /// </summary>
    readonly Dictionary<string, ItemPool> mItemPoolDict = new Dictionary<string, ItemPool>();

    /// <summary>
    /// 对象池列表
    /// </summary>
    readonly List<ItemPool> mItemPoolList = new List<ItemPool>();
    ...
    readonly List<LoopListViewItem2> mItemList = new List<LoopListViewItem2>();
    ...
    protected bool mIsVertList = false;
    System.Func<LoopListView2, int, LoopListViewItem2> mOnGetItemByIndex;
    readonly Vector3[] mItemWorldCorners = new Vector3[4];
    readonly Vector3[] mViewPortRectLocalCorners = new Vector3[4];
    ...
    /// <summary>
    /// 顶部第一个item的下面顶点超出顶部回收到池的距离
    /// 要大于新建距离
    /// </summary>
    float mDistanceForRecycle0 = 300;
    /// <summary>
    /// 顶部第一个item的顶部顶点距离顶部新建一个item的距离
    /// 大于该距离不会新建item
    /// </summary>
    float mDistanceForNew0 = 200;
    /// <summary>
    /// 底部最后个item的上面顶点超出底部回收到池的距离
    /// 要大于新建距离
    /// </summary>
    float mDistanceForRecycle1 = 300;
    /// <summary>
    /// 底部最后个item的底部顶点距离底部新建一个item的距离
    /// 大于该距离不会新建item
    /// </summary>
    ...
    public System.Action<LoopListView2> mOnBeginDragAction = null;
    public System.Action<LoopListView2> mOnDragingAction = null;
    public System.Action<LoopListView2> mOnEndDragAction = null;
    ...
    public ItemPrefabConfData GetItemPrefabConfData(string prefabName)
    {
        foreach (ItemPrefabConfData data in mItemPrefabDataList)
        {
            ...
            {
                SDDebug.LogError("A item prefab is null, 请检查预制件中的ItemPrefabName项。Item名为" + prefabName + "的预制未找到！");
                continue;
            }
    ...
    */
        ...
        LoopListViewInitParam initParam = null)
    {
        if (mListViewInited)
        {
            SDDebug.LogError("LoopListView2.InitListView method can be called only once.");
            return;
        }
        if (initParam != null)
        {
        ...
        mScrollRect = mScrollRect ?? gameObject.GetComponent<ScrollRect>();
        if (mScrollRect == null)
        {
            SDDebug.LogError("ListView Init Failed! ScrollRect component not found!");
            return;
        }
        if (mDistanceForRecycle0 <= mDistanceForNew0)
        {
            SDDebug.LogError("mDistanceForRecycle0 should be bigger than mDistanceForNew0");
        }
        if (mDistanceForRecycle1 <= mDistanceForNew1)
        {
            SDDebug.LogError("mDistanceForRecycle1 should be bigger than mDistanceForNew1");
        }
        mCurSnapData.Clear();
        mItemPosMgr = new ItemPosMgr(mItemDefaultWithPaddingSize, mListStartOffset);
        ...
        {
            SDDebug.LogError("ScrollRect.horizontalScrollbarVisibility cannot be set to AutoHideAndExpandViewport");
        }
        ...
        {
            SDDebug.LogError("ScrollRect.verticalScrollbarVisibility cannot be set to AutoHideAndExpandViewport");
        }
        ...
        AdjustContainerAnchor(mContainerTrans);
        AdjustContainerPivot(mContainerTrans);
        InitItemPool(itemClickFun, this);
        mOnGetItemByIndex = onGetItemByIndex;
        mListViewInited = true;
        ResetListView();
        FillSuperList(itemTotalCount, true);
    }

    /// <summary>
    /// 支持多个itemPrefab
    /// 重载InitListView方法 因为给Lua调用和参数的关系 所以改个名字
    /// 不会调用渲染函数FillSuperList
    /// </summary>
    /// <param name="onGetItemByIndex"></param>
    /// <param name="initParam"></param>
    public void InitListViewNoFill(
        System.Func<LoopListView2, int, LoopListViewItem2> onGetItemByIndex,
        LoopListViewInitParam initParam = null)
    {
        if (mListViewInited)
    ...
    /// 单个渲染函数 使用的
    /// </summary>
    /// <param name="itemRendener"></param>
    /// <param name="initParam"></param>
    public void InitListViewSingle(ItemRendener itemRendener, LoopListViewInitParam initParam = null)
    {
        if (mListViewInited)
        {
            SDDebug.LogError("LoopListView2.InitListView method can be called only once.");
        ...
        AdjustContainerPivot(mContainerTrans);
        //add start
        if (null != scrollRectChgValue)
        {
            mScrollRect.onValueChanged.RemoveAllListeners();
            mScrollRect.onValueChanged.AddListener((vector) =>
            {
                scrollRectChgValue(this.ScrollValue(), vector.x, vector.y);
            });
        }

        if (mIsVertList)
        {
            mContainerTrans.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, mViewPortRectTransform.rect.width);
        }
        else
        {
            mContainerTrans.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, mViewPortRectTransform.rect.height);
        }
        //end

        float itemSize = InitItemPool(itemClickFun, this);
        if (initParam != null)
        {
            mDistanceForRecycle0 = initParam.mDistanceForRecycle0;
            mDistanceForNew0 = initParam.mDistanceForNew0;
        ...
        {
            // add
            mDistanceForRecycle0 = itemSize > 100 ? itemSize + 100 : itemSize + 30;
            mDistanceForNew0 = itemSize;
            mDistanceForRecycle1 = itemSize > 100 ? itemSize + 100 : itemSize + 30;
            mDistanceForNew1 = itemSize;
        }
        if (mDistanceForRecycle0 <= mDistanceForNew0)
        {
            SDDebug.LogError("mDistanceForRecycle0 should be bigger than mDistanceForNew0");
        ...
        this.itemRendener = itemRendener;
        mListViewInited = true;
        ResetListView();
    }
    ...
    void SetScrollbarListener()
    {
        ...
        listener.onPointerUp += OnPointerUpInScrollBar;
        listener.onPointerDown += OnPointerDownInScrollBar;
    }
    ...
    此方法可用于在运行时设置滚动视图的项目总数。
        如果此参数设置为-1，则表示存在无限项目，
        并且滚动条不被支持，并且ItemIndex可以从-MaxInt到+MaxInt。
        如果此参数设置值> = 0，则ItemIndex只能为0到itemTotalCount -1。
        如果resetPos设置为false，则此方法完成后，scrollrect的内容位置不会更改。
    */
    public void FillSuperList(int numItems, bool resetPos = true)
    {
        //====================================
        numDatas = numItems;
        // 参数是渲染数据的总数 因为Grid一个item会渲染多条数据 所以计算逻辑上真实的numItems
        int totalItems = LogicNumItems(numItems);
        //====================================
        if (totalItems == mItemTotalCount)
        {
            // 同一个列表会多次渲染 比如切换页签
            //return;
        }
        mCurSnapData.Clear();
        mItemTotalCount = totalItems;
    ...
    /// Add Fun 滑动到某一个item
    /// </summary>
    /// <param name="isNeedRefill">是否需要重新填充List</param>
    /// <param name="toIndex">目标item索引</param>
    /// <param name="numItems">List item总数 isNeedRefill = true才有意义</param>
    public void SuperListScrollTo(bool isNeedRefill, int toIndex, int numItems = 0)
    {
        if (isNeedRefill)
        {
            numDatas = numItems;
        ...
        MovePanelToItemIndex(toIndex, 0);
    }
    ...
    */
    ...
    {
        //====================================
        int logicIndex = LogicMovePanelItemIndex(itemIndex);
        //====================================
        mScrollRect.StopMovement();
        mCurSnapData.Clear();
        if (logicIndex < 0 || mItemTotalCount == 0)
        {
            return;
        }
        if (mItemTotalCount > 0 && logicIndex >= mItemTotalCount)
        {
            logicIndex = mItemTotalCount - 1;
        }
        if (offset < 0)
        {
            offset = 0;
        ...
        RecycleAllItem();
        LoopListViewItem2 newItem = GetNewItemByIndex(logicIndex);
        if (newItem == null)
        {
            ClearAllTmpRecycledItem();
            return;
        ...
        if (mSupportScrollBar)
        {
            if (mIsVertList)
            {
                SetItemSize(logicIndex, newItem.CachedRectTransform.rect.height, newItem.Padding);
            }
            else
            {
                SetItemSize(logicIndex, newItem.CachedRectTransform.rect.width, newItem.Padding);
            }
        }
        mItemList.Add(newItem);
        UpdateContentSize();
        //UpdateListView(viewPortSize + 100, viewPortSize + 100, viewPortSize, viewPortSize);
        // 修改 这里做个优化 在初始化列表参数的时候 会把参数填好保证创建出来的item不会立即又被回收了
        UpdateListView(viewPortSize + 100, viewPortSize + 100, mDistanceForNew0, mDistanceForNew1);
        AdjustPanelPos();
        ClearAllTmpRecycledItem();
    }
    ...
    float InitItemPool(Action<GameObject, LoopListView2> itemClickFun, LoopListView2 list)
    {
        foreach (ItemPrefabConfData data in mItemPrefabDataList)
        {
            //====================================
            if (ReferenceEquals(data.mItemPrefab, null))
            {
                SDDebug.LogError("A item prefab is null ");
                continue;
            }
            if (list.mIsGrid && !data.mIsItemGroup)
            {
                var item = data.mItemPrefab;
                var mItemPrefab = new GameObject(item.name + "Group");
                var rect = mItemPrefab.AddComponent<RectTransform>();
                rect.anchorMax = Vector2.up;
                rect.anchorMin = Vector2.up;
                rect.pivot = Vector2.up;

                // 计算一组item个数
                int itemCount = list.mItemCountPerCell;
                ...
                if (list._isVariable)
                {
                    if (list.mIsVertList)
                    {
                        itemCount = Mathf.FloorToInt((listRect.rect.size.x + list.mItemGroupSpacing - data.mStartPosOffset) /
                                         (itemRect.rect.size.x + list.mItemGroupSpacing));
                    }
                    else
                    {
                        itemCount = Mathf.FloorToInt((listRect.rect.size.y + list.mItemGroupSpacing - data.mStartPosOffset) /
                                         (itemRect.rect.size.y + list.mItemGroupSpacing));
                    }
                    list.mItemCountPerCell = itemCount;
                }

                // 因为即时刷新布局API并没有即时刷新布局，所以自己算mItemPrefab的大小，也不用ContentSizeFitter组件了
                var v = Vector2.zero;
                v.x = (itemRect.rect.size.x + list.mItemGroupSpacing) * itemCount - list.mItemGroupSpacing;
                v.y = itemRect.rect.size.y;
                rect.sizeDelta = v;
                ...
                layout.spacing = list.mItemGroupSpacing;
                layout.childAlignment = TextAnchor.UpperLeft;
                layout.childControlHeight = false;
                layout.childControlWidth = false;
                layout.childForceExpandHeight = false;
                layout.childForceExpandWidth = false;
                //var sizeFitter = rect.gameObject.AddComponent<ContentSizeFitter>();
                //sizeFitter.horizontalFit = ContentSizeFitter.FitMode.PreferredSize;
                //sizeFitter.verticalFit = ContentSizeFitter.FitMode.PreferredSize;

                List<GameObject> l = new List<GameObject>();
                var uibase = rect.gameObject.AddComponent<UIBase>();
                for (int i = 0; i < itemCount; i++)
                {
                    l.Add(Instantiate(item, mItemPrefab.transform));
                }
                // 即时刷一下布局
                LayoutRebuilder.ForceRebuildLayoutImmediate(rect);
                uibase.AddUIBaseItem(l);
                data.mItemPrefab = mItemPrefab;
                // 这里设置一下居中
                if (list._isVariable)
                {
                    if (list.mIsVertList)
                    {
                        data.mStartPosOffset += (listRect.rect.size.x - data.mStartPosOffset - rect.rect.size.x) / 2;
                    }
                    else
                    {
                        data.mStartPosOffset += (listRect.rect.size.y - data.mStartPosOffset - rect.rect.size.y) / 2;
                    }
                }

                mItemPrefab.SetParentEx(mContainerTrans);
            }

            //====================================
            if (data.mItemPrefab == null)
            {
                SDDebug.LogError("A item prefab is null， 请检查预制件中的ItemPrefabName项。Item 的预制未找到！ ");
                continue;
            }
            string prefabName = data.mItemPrefab.name;
            if (mItemPoolDict.ContainsKey(prefabName))
            {
                SDDebug.LogError("A item prefab with name " + prefabName + " has existed!");
                continue;
            }
            RectTransform rtf = data.mItemPrefab.GetComponent<RectTransform>();
            if (rtf == null)
            {
                SDDebug.LogError("RectTransform component is not found in the prefab " + prefabName);
                continue;
            }
            AdjustAnchor(rtf);
            AdjustPivot(rtf);
            // 不在这里添加LoopListViewItem2，因为要影响prefab。放到实例化出来后添加
            ...
            ItemPool pool = new ItemPool(itemClickFun, list.mIsGrid, list);
            pool.Init(data.mItemPrefab, data.mPadding, data.mStartPosOffset, data.mInitCreateCount, mContainerTrans);
            mItemPoolDict.Add(prefabName, pool);
            mItemPoolList.Add(pool);
            if (mItemPrefabDataList.Count == 1)
            {
                return mIsVertList ? rtf.sizeDelta.y : rtf.sizeDelta.x;
            }
        }
        return 0;
    }
    ...
    public virtual void OnEndDrag(PointerEventData eventData)
    {
        ...
        {
            mOnEndDragAction(this);
        }
        ForceSnapUpdateCheck();
        //CheckVerticalDragBottom();
    }
    ...
    LoopListViewItem2 GetNewItemByIndex(int index)
    {
        ...
        if (newItem == null)
        {
            //return null;
            //单个类型的item
            var enu = mItemPoolDict.GetEnumerator();
            string itemPrefabName = null;
            while (enu.MoveNext())
            {
                // 这里只取第一个 也只会有一个
                itemPrefabName = enu.Current.Key;
                break;
            }
            newItem = NewListViewItem(itemPrefabName);
            if (newItem == null)
            {
                return null;
            }
            ...
            newItem.RecycleFunc = null;
            if (!mIsGrid)
            {
                if (this.itemRendener != null)
                {
                    this.itemRendener(index, newItem.gameObject, index);
                }
            }
            UIBase uibase = newItem.gameObject.GetComponent<UIBase>();
            if (!uibase)
            {
                SDDebug.LogError("called GetNewItemByIndex but get UIBase is null from newItem!");
                return null;
            }
            if (mIsGrid)
            {
                for (int i = 0; i < mItemCountPerCell; ++i)
                {
                    GameObject go = uibase.TargetContainer[i].goNode;
                    int itemIndex = index * mItemCountPerCell + i;
                    if (itemIndex >= numDatas)
                    {
                        go.SetActive(false);
                        continue;
                    }
                    ...
                    if (this.itemRendener != null)
                    {
                        this.itemRendener(itemIndex, go, index);
                    }
                    LoopListViewItem2 viewItem = go.GetOrAddComponent<LoopListViewItem2>();
                    viewItem.ItemGroupIndex = index;
                    viewItem.ItemIndex = itemIndex;
                    viewItem.RecycleFunc = null;
                }
            }
        }
        newItem.ItemGroupIndex = index; // Add
    ...
    void AdjustPanelPos()
    {
        ...
        if (mArrangeType == ListItemArrangeType.TopToBottom)
        {
            if (contentSize <= viewPortSize)
            {
                ...
                // 这里是做item离顶或底有一段距离的需求 下同
                mItemList[0].CachedRectTransform.anchoredPosition3D = new Vector3(mItemList[0].StartPosOffset, -mListStartOffset, 0);
                UpdateAllShownItemsPos();
                return;
            }
        ...
        else if (mArrangeType == ListItemArrangeType.LeftToRight)
        {
            if (contentSize <= viewPortSize)
            {
                ...
                mItemList[0].CachedRectTransform.anchoredPosition3D = new Vector3(mListStartOffset, mItemList[0].StartPosOffset, 0);
                UpdateAllShownItemsPos();
                return;
            }
            LoopListViewItem2 tViewItem0 = mItemList[0];
    ...
    void UpdateSnapHorizontal(bool immediate = false)
    {
        ...
        if (needCheck)
        {
            ...
            else if (mArrangeType == ListItemArrangeType.LeftToRight)
            {
                ...
                itemSnapCenter = start + tViewItem0.ItemSize * mItemSnapPivot.x;
                //SDDebug.LogErrorFormat("item name = {0}",tViewItem0.gameObject.name);
                for (int i = 0; i < count; ++i)
                {
                    curDist = snapCenter - itemSnapCenter;
                    curDistAbs = Mathf.Abs(curDist);
    ...
    /// <summary>
    /// 注意 回收距离要大于新建距离 不然创建出来 又会被回收 导致一些显示bug
    /// </summary>
    /// <param name="distanceForRecycle0">顶部第一个item的下面顶点超出顶部回收到池的距离</param>
    /// <param name="distanceForRecycle1">底部最后个item的上面顶点超出底部回收到池的距离</param>
    /// <param name="distanceForNew0">顶部第一个item的顶部顶点距离顶部新建一个item的距离</param>
    /// <param name="distanceForNew1">底部最后个item的底部顶点距离底部新建一个item的距离</param>
    public void UpdateListView(float distanceForRecycle0, float distanceForRecycle1, float distanceForNew0, float distanceForNew1)
    {
        ...
        if (mIsVertList)
        {
            ...
            while (needContinueCheck)
            {
                ...
                {
                    SDDebug.LogError("UpdateListView Vertical while loop " + checkCount + " times! something is wrong!");
                    break;
                }
        ...
        else
        {
            ...
            while (needContinueCheck)
            {
                ...
                if (checkCount >= maxCount)
                {
                    SDDebug.LogError("UpdateListView  Horizontal while loop " + checkCount + " times! something is wrong!");
                    break;
                }
    ...
    float GetContentPanelSize()
    {
        if (mSupportScrollBar)
        {
            ...
            //修改
            return tTotalSize + mListStartOffset + mListEndOffset;
        }
        int count = mItemList.Count;
        if (count == 0)
        {
        ...
        if (count == 1)
        {
            return mItemList[0].ItemSize + mListStartOffset + mListEndOffset;
        }
        if (count == 2)
        {
            return mItemList[0].ItemSizeWithPadding + mItemList[1].ItemSize + mListStartOffset + mListEndOffset;
        }
        float s = 0;
        ...
        //修改
        return s + mListStartOffset + mListEndOffset;
    }
    ...
    void CheckIfNeedUpdataItemPos()
    {
        ...
        if (mArrangeType == ListItemArrangeType.TopToBottom)
        {
            ...
            //修改
            //var condition1 = firstItem.TopY > 0 || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.TopY != 0);
            var condition1 = firstItem.TopY > -mListStartOffset || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.TopY != -mListStartOffset);
            if (condition1)
            {
                UpdateAllShownItemsPos();
                return;
            }
            //var condition2 = -lastItem.BottomY > viewMaxY || (lastItem.ItemIndex == mCurReadyMaxItemIndex && -lastItem.BottomY != viewMaxY);
            var condition2 = -lastItem.BottomY > viewMaxY - mListEndOffset || (lastItem.ItemIndex == mCurReadyMaxItemIndex && -lastItem.BottomY != viewMaxY - mListEndOffset);
            if (condition2)
            {
                UpdateAllShownItemsPos();
                return;
        ...
        else if (mArrangeType == ListItemArrangeType.BottomToTop)
        {
            ...
            LoopListViewItem2 lastItem = mItemList[mItemList.Count - 1];
            float viewMaxY = GetContentPanelSize();
            //var condition1 = firstItem.BottomY < 0 || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.BottomY != 0);
            var condition1 = firstItem.BottomY < mListStartOffset || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.BottomY != mListStartOffset);
            if (condition1)
            {
                UpdateAllShownItemsPos();
                return;
            }
            //var condition2 = lastItem.TopY > viewMaxY || (lastItem.ItemIndex == mCurReadyMaxItemIndex && lastItem.TopY != viewMaxY);
            var condition2 = lastItem.TopY > viewMaxY - mListEndOffset || (lastItem.ItemIndex == mCurReadyMaxItemIndex && lastItem.TopY != viewMaxY - mListEndOffset);
            if (condition2)
            {
                UpdateAllShownItemsPos();
                return;
        ...
        else if (mArrangeType == ListItemArrangeType.LeftToRight)
        {
            ...
            //var condition1 = firstItem.LeftX < 0 || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.LeftX != 0);
            var condition1 = firstItem.LeftX < mListStartOffset || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.LeftX != mListStartOffset);
            if (condition1)
            {
                UpdateAllShownItemsPos();
                return;
            }
            //var condition2 = lastItem.RightX > viewMaxX || (lastItem.ItemIndex == mCurReadyMaxItemIndex && lastItem.RightX != viewMaxX);
            var condition2 = lastItem.RightX > viewMaxX - mListEndOffset || (lastItem.ItemIndex == mCurReadyMaxItemIndex && lastItem.RightX != viewMaxX - mListEndOffset);
            if (condition2)
            {
                UpdateAllShownItemsPos();
                return;
        ...
        else if (mArrangeType == ListItemArrangeType.RightToLeft)
        {
            ...
            LoopListViewItem2 lastItem = mItemList[mItemList.Count - 1];
            float viewMaxX = GetContentPanelSize();
            //var condition1 = firstItem.RightX > 0 || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.RightX != 0);
            var condition1 = firstItem.RightX > -mListStartOffset || (firstItem.ItemIndex == mCurReadyMinItemIndex && firstItem.RightX != -mListStartOffset);
            if (condition1)
            {
                UpdateAllShownItemsPos();
                return;
            }
            //var condition2 = -lastItem.LeftX > viewMaxX || (lastItem.ItemIndex == mCurReadyMaxItemIndex && -lastItem.LeftX != viewMaxX);
            var condition2 = -lastItem.LeftX > viewMaxX - mListEndOffset || (lastItem.ItemIndex == mCurReadyMaxItemIndex && -lastItem.LeftX != viewMaxX - mListEndOffset);
            if (condition2)
            {
                UpdateAllShownItemsPos();
                return;
    ...
    void UpdateAllShownItemsPos()
    {
        ...
        // 这里限制一下最大速度
        mAdjustedVec.x = Mathf.Clamp(mAdjustedVec.x, -scrollRectMaxVec, scrollRectMaxVec);
        mAdjustedVec.y = Mathf.Clamp(mAdjustedVec.y, -scrollRectMaxVec, scrollRectMaxVec);
        ...
        if (mIsDraging)
        {
            ...
            mScrollRect.velocity = mAdjustedVec;
            //SDDebug.LogErrorFormat("UpdateAllShownItemsPos mScrollRect velocity = {0}", mAdjustedVec);
            mNeedAdjustVec = true;
        }
    }
